...
func TestErrors(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			`gf/html/template:z: "=" in unquoted attr: "onclick="`,
		},
		{
			`<input type=button value= onclick=>`,
			`gf/html/template:z: "=" in unquoted attr: "onclick="`,
		},
		{
			...
			`gf/html/template:z: "=" in unquoted attr: "1+1=2"`,
		},
		{
			"<a class=`foo>",
		...
		{
			`<a style=font:'Arial'>`,
			`gf/html/template:z: "'" in unquoted attr: "font:'Arial'"`,
		},
		{
			`<a=foo>`,
...
// Unlike gf/text/template, gf/html/template crashed if given an incomplete
